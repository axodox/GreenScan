...
namespace Green
{
	namespace Kinect
	{
		...
		private:
			...
			bool WorkerThreadOn, SaveInProgress;
		...
		private:
			unsigned ContinousShootingCounter;
			bool ContinousShooting;
			LPWSTR ContinousShootingPath, ContinousShootingMetadata;
			HANDLE ContinousShootingThread, ContinousShootingTimer;
			static DWORD WINAPI ContinousShootingWorker(LPVOID o)
			{
				wchar_t path[MAX_PATH];
				KinectDevice* kd = (KinectDevice*)o;
				while (kd->ContinousShooting)
				{
					wsprintf(path, L"%s_%04i.gsr", kd->ContinousShootingPath, kd->ContinousShootingCounter++);
					kd->SaveRaw(path, kd->ContinousShootingMetadata);
					WaitForSingleObject(kd->ContinousShootingTimer, INFINITE);
				}

				CloseHandle(kd->ContinousShootingTimer);
				LPWSTRDelete(kd->ContinousShootingPath);
				LPWSTRDelete(kd->ContinousShootingMetadata);
				return 0;
			}
		public:
			bool StartContinousShooting(LPWSTR path, LPWSTR metadata, int interval)
			{
				if (ContinousShooting) return false;

				ContinousShooting = true;
				ContinousShootingCounter = 0;
				ContinousShootingPath = new wchar_t[MAX_PATH];
				wcscpy(ContinousShootingPath, path);
				ContinousShootingMetadata = new wchar_t[wcslen(metadata) + 1];
				wcscpy(ContinousShootingMetadata, metadata);

				ContinousShootingTimer = CreateWaitableTimer(0, 0, 0);

				FILETIME ft;
				SYSTEMTIME st;
				GetSystemTime(&st);
				SystemTimeToFileTime(&st, &ft);
				SetWaitableTimer(ContinousShootingTimer, (LARGE_INTEGER*)&ft, interval, nullptr, nullptr, false);

				ContinousShootingThread = CreateThread(0, 0, &ContinousShootingWorker, this, 0, 0);
				return true;
			}

			unsigned GetContinousShootingCount()
			{
				return ContinousShootingCounter;
			}

			void StopContinousShooting()
			{
				if (!ContinousShooting) return;
				ContinousShooting = false;
				WaitForSingleObject(ContinousShootingThread, INFINITE);
				CloseHandle(ContinousShootingThread);
			}
